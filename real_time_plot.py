...
class AnimationPlot:
    ...
    def animate(self, i, dataList, ser):
        ...
        arduinoData_string = ser.readline().decode() # Decode receive Arduino data as a formatted string
        arduinoData_vector = arduinoData_string.split(";") 
        #print(i)                                           # 'i' is a incrementing variable based upon frames = x argument

        try:
            arduinoData_float = float(arduinoData_vector[i].split(":")[1])   # Convert to float
